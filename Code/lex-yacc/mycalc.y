...
%token NOT POT
%token MINUS PLUS MOD MULT DIV MINUSMINUS PLUSPLUS //ARITMETIC
...
statement:
  instruction ';' { $$ = insert_statement(d_instruction, $1); }
...
break_cont:
  BREAK { $$ = insert_break_cont(is_BREAK); }
| CONTINUE { $$ = insert_break_cont(is_CONTINUE); };

concat:
  CONCATSYMB '(' concat_args ')' { $$ = insert_concat($3); };

concat_args:
  type ',' concat_args  { $$ = insert_concat_args($1, NULL, $3); }
| type ',' type { $$ = insert_concat_args($1, $3, NULL); };

read_statement:
  READ '(' ID ')' { $$ = insert_read_statement(d_READ, $3); }
| READLINE '(' ID ')' { $$ = insert_read_statement(d_READLINE, $3); };

instruction:
  expression { $$ = insert_instruction(d_expression, $1); }
| atribution { $$ = insert_instruction(d_atribution, $1); }
| plusminus { $$ = insert_instruction(d_plusminus, $1); };

plusminus:
  ID PLUSPLUS { $$ = insert_plusminus(d_PLUSPLUS, $1); }
| ID MINUSMINUS { $$ = insert_plusminus(d_MINUSMINUS, $1); };

compound_statement:
  if_statement { $$ = insert_compound_statement(d_if_statement, $1); }
| while_statement { $$ = insert_compound_statement(d_while_statement, $1); }
| for_statement { $$ = insert_compound_statement(d_for_statement, $1); };

println_statement:
  PRINTLN '(' println_args ')' { $$ = insert_println_statement(d_PRINTLN, $3); }
| PRINT '(' println_args ')' { $$ = insert_println_statement(d_PRINT, $3); };

println_args:
  type PLUS println_args { $$ = insert_println_args($1, $3); }
| type { $$ = insert_println_args($1, NULL); };

return_statement:
  RETURNSYMB { $$ = insert_return_statement(NULL); }
| RETURNSYMB instruction{ $$ = insert_return_statement($2); };

for_statement:
  FOR '(' for_args ';' relac_exp ';' for_args ')' cont_statement { $$ = insert_for_statement($3, $5, $7, $9); };

for_args:
  dec_list { $$ = insert_for_args(d_for_dec_list, $1); }
| declaration { $$ = insert_for_args(d_for_declaration, $1); }
| plusminus { $$ = insert_for_args(d_for_plusminus, $1); }
| { $$ = NULL; };

if_statement:
  IF '(' instruction ')' cont_statement elseif_statement_list else_statement { $$ = insert_if_statement($3, $5, $6, $7); };

while_statement:
  WHILE '(' instruction ')' cont_statement { $$ = insert_while_statement($3, $5); };

else_statement:
  ELSE cont_statement { $$ = insert_else_statement($2); }
| { $$ = NULL; };

elseif_statement_list:
  elseif_statement_list elseif_statement{ $$ = insert_elseif_statement_list($1, $2); }
| { $$ = NULL; };

elseif_statement:
  ELSEIF '(' instruction ')' cont_statement { $$ = insert_elseif_statement($3, $5); };

cont_statement:
  statement { $$ = insert_cont_statement(d_statement, $1); }
| '{' statement_list '}' { $$ = insert_cont_statement(d_statement_list, $2); }
| '{' '}' { $$ = NULL; }
| ';' { $$ = NULL; };

func_def:
  FUNCSYMB symbols ID '(' arg_list ')' cont_func { $$ = insert_func_def($2, $3, $5, $7); };
...
cont_func:
  '{' statement_list '}' { $$ = insert_cont_func($2); }
| '{' '}' { $$ = NULL; };

arg_list:
  arg_list ',' symbols ID{ $$ = insert_arg_list($1, $3, $4); }
| symbols ID{$$ = insert_arg_list(NULL, $1, $2);}
| { $$ = NULL; };

symbols:
  INTSYMB { $$ = insert_symbols(d_INTEGER); }
...
declaration:
  INTSYMB dec_list{ $$ = insert_declaration(d_INTEGER, $2); }
...
dec_list:
  atr { $$ = insert_dec_list(NULL, $1); }
| dec_list ',' atr { $$ = insert_dec_list($1, $3); };

atr:
  ID { $$ = insert_atr(d_ID, $1); }
| atribution { $$ = insert_atr(d_atribution_atr, $1); };

atribution:
  atribution_list expression{ $$ = insert_atribution($1, $2); };

atribution_list:
  atribution_list ID EQUAL { $$ = insert_atribution_list(d_EQUAL, $1, $2); }
...
expression:
  exp_minus_plus { $$ = insert_expression(d_exp_minus_plus, $1); }
...
exp_minus_plus:
  MINUS expression{ $$ = insert_exp_minus_plus($2, is_MINUS); }
| PLUS expression{ $$ = insert_exp_minus_plus($2, is_PLUS); };

logic_exp_not:
  NOT expression{ $$ = insert_logic_exp_not($2); };

logic_exp_or_and:
  expression AND expression { $$ = insert_logic_exp_or_and($1, is_AND, $3); }
| expression OR expression { $$ = insert_logic_exp_or_and($1, is_OR, $3); };

relac_exp:
  expression EQUALEQUAL expression { $$ = insert_relac_exp($1, is_EQUAL, $3); }
...
aritmetic_exp:
  expression PLUS expression { $$ = insert_aritmetic_exp($1, is_PLUS, $3); }
