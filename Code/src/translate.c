...
void translate(is_program *ip) {
    ...
    Int_node *element = (Int_node *) malloc(sizeof(Int_node));
    Int_node *element2 = (Int_node *) malloc(sizeof(Int_node));
    Char_node *element3 = (Char_node *) malloc(sizeof(Char_node));
...
void translate_global_dec_type(FILE *dest, disc_type type, int offset) {
    ...
    switch (type) {
        case d_INTEGER:
            fprintf(dest, "int g%d=0;\n", offset);
            break;
        case d_STRING:
            fprintf(dest, "char* g%d=\"\";\n", offset);
            break;
        case d_DOUBLE:
            fprintf(dest, "double g%d=0;\n", offset);
            break;
        case d_FLOAT:
            fprintf(dest, "float g%d=0;\n", offset);
            break;
        case d_BOOL:
            fprintf(dest, "int g%d=0;\n", offset);
            break;
...
void translate_local_dec_type(FILE *dest, table_element *t, disc_type dt) {
    ...
    switch (dt) {
        ...
        case d_DOUBLE:
            ...
            break;
        case d_FLOAT:
            fprintf(dest, "sp->locals[%d]=(float*)malloc(sizeof(float));\n", t->offset);
...
void translate_read_statement(FILE *dest, environment_list *el, prog_env *pe, is_read_statement *irs) {
    ...
    if (irs->disc_d == d_READLINE) {
        translate_readln_statement(dest, newel, flag);
        return;
    }
...
void translate_ourgoing_dec_type(FILE *dest, table_element *t, disc_type dt) {
    ...
    switch (dt) {
        case d_INTEGER:
            fprintf(dest, "sp->outgoing[%d]=(int*)malloc(sizeof(int));\n", t->offset);
            break;
        case d_STRING:
            fprintf(dest, "sp->outgoing[%d]=(char*)malloc(sizeof(char));\n", t->offset);
            break;
        case d_DOUBLE:
            fprintf(dest, "sp->outgoing[%d]=(double*)malloc(sizeof(double));\n", t->offset);
            break;
        case d_FLOAT:
            fprintf(dest, "sp->outgoing[%d]=(float*)malloc(sizeof(float));\n", t->offset);
            break;
        case d_BOOL:
            fprintf(dest, "sp->outgoing[%d]=(int*)malloc(sizeof(int));\n", t->offset);
            break;
...
void create_return_value(FILE *dest, disc_type ty) {
    switch (ty) {
        case d_INTEGER:
            fprintf(dest, "return_value[_ra]=(int*)malloc(sizeof(int));\n");
            fprintf(dest, "*((int*)return_value[_ra])=  ");
            break;
        case d_STRING:
            fprintf(dest, "return_value[_ra]=(char*)malloc(sizeof(char));\n");
            fprintf(dest, "return_value[_ra]= ");
            break;
        case d_DOUBLE:
            fprintf(dest, "return_value[_ra]=(double*)malloc(sizeof(double));\n");
            fprintf(dest, "*((double*) return_value[_ra]=  ");
            break;
        case d_FLOAT:
            fprintf(dest, "return_value[_ra]=(float*)malloc(sizeof(float));\n");
