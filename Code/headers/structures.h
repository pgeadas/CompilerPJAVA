#ifndef PJAVA_STRUCTURES_H
#define PJAVA_STRUCTURES_H
...
typedef enum {
    d_atribution, d_expression, d_plusminus
} disc_instr;
...
typedef enum {
    d_EQUAL, d_MINUSEQUAL, d_PLUSEQUAL
} disc_atr_lis;

//is_expression --> is_logic_oper_or_and V is_logic_oper_not V is_aritmetic_oper V is_relac_oper
typedef enum {
    d_logic_exp_or_and, d_logic_exp_not, d_exp_minus_plus, d_aritmetic_exp, d_relac_exp, d_type, d_expression_exp
} disc_expr;

//is_oper_a --> is_PLUS ∨ is_MINUS v is_MULT v is_DIV v is_MOD v is_POT
typedef enum {
    is_PLUS, is_MINUS, is_MULT, is_DIV, is_MOD, is_POT
} is_oper_a;
...
typedef enum {
    is_GREATER, is_GREATEREQUAL, is_LESS, is_LESSEQUAL, is_EQUAL, is_NOTEQUAL
} is_oper_r;

// is_compound_statement --> is_if_statement V is_while_statement V is_stmt_for
typedef enum {
    d_if_statement, d_while_statement, d_for_statement
} disc_compound_statement;

typedef enum {
    d_statement, d_statement_list
} disc_cont_statement;

typedef enum {
    d_atribution_atr, d_ID
} disc_atr;

typedef enum {
    d_for_dec_list, d_for_declaration, d_for_plusminus
} disc_for_args;
