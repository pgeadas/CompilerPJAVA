...
{
	pe=(prog_env*)malloc(sizeof(prog_env));  //criação do program_environment, que contera todas os símbolos (globais, locais, procedimentos)
...
//DEC LIST
int semantic_analysis_dec_list(int offset, int scope, prog_env *pe, table_element *te, is_dec_list* idl, disc_type dt, int caller)
{
	is_dec_list *tmp;
...
//ATR
int semantic_analysis_atr(int offset, int scope, prog_env *pe, table_element *te, is_atr* ics, disc_type dt, int caller)
{
	table_element* newel;
...
//ATRIBUTION
int semantic_analysis_is_atribution(int caller, int offset, int scope, prog_env *pe, table_element *te, disc_type dt, is_atribution* ia)
{
	tipoActual=-1;
...
//ATRIB LIST
int semantic_analysis_is_atrib_list(int scope, prog_env *pe, table_element *te, disc_type dt, is_atribution_list *ia, int caller, int offset){
	...
	for(atrib_aux=ia; atrib_aux; atrib_aux=atrib_aux->next){
			...
			if(newel==NULL){
				...
				if(newel==NULL){
					...
					if(newel->type!=tipoActual){
						printf("ERRO NA LINHA %d | (ATRIB_LIST) Types mismatch (%s<->%s)!\n", id_tmp->codeLine, typeToString(newel->type), typeToString(tipoActual));
						totalErros++;	
					}
		...
		{	
			if(dt!=tipoActual && (dt!=d_INTEGER && dt!=d_DOUBLE && dt!=d_FLOAT) && (tipoActual!=d_INTEGER && tipoActual!=d_DOUBLE && tipoActual!=d_FLOAT)){
	 			printf("ERRO NA LINHA %d | (ATRIB_LIST) Types mismatch (%s<->%s)!\n", id_tmp->codeLine, typeToString(dt), typeToString(tipoActual));
				totalErros++;	
				if(scope==GLOBALSCOPE)
					global_offset--;
				else
...
char* typeToString(disc_type type)
{
	...
	case d_type_ID: return "ID";
	case d_type_func_call: return "Func";
...
//ARG LIST
int semantic_analysis_arg_list(int offset, int scope, prog_env *pe, table_element* locals, table_element* arguments, is_arg_list *ial)
{
	is_arg_list* tmp;
	...
	for(tmp=ial; tmp; tmp=tmp->next){
		semantic_analysis_symbols((scope==LOCALSCOPE?offset++:global_offset++), pe, locals, arguments, tmp->sym, tmp->id);
	}
...
//CONT FUNC
void semantic_analysis_is_cont_func(environment_list* actualFunc, prog_env *pe, table_element* locals, is_cont_func* icf)
{
	if(icf==NULL){
		return;
	}
...
//STATEMENT LIST
void semantic_analysis_is_statement_list(environment_list* actualFunc, prog_env *pe, table_element* locals, is_statement_list* isl)
{
	is_statement_list* tmp;
...
//STATEMENT
void semantic_analysis_is_statement(environment_list* actualFunc, prog_env *pe, table_element* locals, is_statement* is)
{	
	aritm=0;
	switch (is->disc_d)
	{
		...
		case d_declaration:
			...
			actualFunc->offset=semantic_analysis_declaration(actualFunc->offset, LOCALSCOPE, pe, locals, is->data_statement.u_declaration);
...
//FUNC CALL
void semantic_analysis_is_func_call(environment_list* actualFunc, prog_env *pe, table_element *locals, is_func_call* ifcall)
{
	int encontrou=0;
	environment_list *aux;
	for(aux=pe->procs; aux; aux=aux->next){
...
//CALL LIST
void semantic_analysis_is_call_list(environment_list* actualFunc, prog_env *pe, table_element *locals, is_call_list* icl)
{
	is_call_list* tmp;
	table_element* aux;
	table_element* aux2;
...
//COMPOUND STATEMENT
void semantic_analysis_is_compound_statement(environment_list* actualFunc, prog_env *pe, table_element* locals, is_compound_statement* ics)
{	
	switch(ics->disc_cs)
	{
		case d_if_statement: 
...
//FOR STATEMENT
void semantic_analysis_is_for_statement(environment_list* actualFunc, prog_env *pe, table_element* locals, is_for_statement* ifs)
{
...
void semantic_analysis_is_for_args(environment_list* actualFunc, prog_env *pe, table_element* locals, is_for_args* ifa)
{
	...
	switch(ifa->disc_n){
		case d_for_declaration:
			actualFunc->offset=semantic_analysis_declaration(actualFunc->offset++, LOCALSCOPE, pe, locals, ifa->data_for_args.u_declaration);
			break;
...
//WHILE STATEMENT
void semantic_analysis_is_while_statement(environment_list* actualFunc, prog_env *pe, table_element* locals, is_while_statement* iws)
{
...
//IF STATEMENT
void semantic_analysis_is_if_statement(environment_list* actualFunc, prog_env *pe, table_element* locals, is_if_statement* iifs)
{
...
//ELSE STATEMENT
void semantic_analysis_is_else_statement(environment_list* actualFunc, prog_env *pe, table_element* locals,is_else_statement* ies)
{
...
//ELSE IF STATMT LIST
void semantic_analysis_is_elseif_statement_list(environment_list* actualFunc, prog_env *pe, table_element* locals, is_elseif_statement_list* ieisl)
{
...
//ELSE IF STATMT
void semantic_analysis_is_elseif_statement(environment_list* actualFunc, prog_env *pe, table_element* locals, is_elseif_statement* ieis)
{
...
//CONT_STATEMENT
void semantic_analysis_is_cont_statement(environment_list* actualFunc, prog_env *pe, table_element* locals, is_cont_statement* ics)
{
	if(ics==NULL){
		return;
	}
...
void semantic_analysis_is_plusminus(prog_env *pe, table_element *te, is_plusminus *ia){
	...
	if(newel==NULL){
		...
		if(newel==NULL){
			...
			if(newel->type==d_VOID || newel->type==d_STRING || newel->type==d_type_ID || newel->type==d_type_func_call){
...
//SYMBOLS
void semantic_analysis_symbols(int offset, prog_env *pe, table_element* locals, table_element* arguments, is_symbols* isy, is_id* id){
